...
def update_available_letters(letters, char):
    updated_letters = letters.replace(char, "")
    return updated_letters


def update_guessed_word(word, guessed_word, char):
    updated_guessed_word = guessed_word
    for i in range(0, len(word)):
        if word[i] == char:
            updated_guessed_word = updated_guessed_word[0:i] + letter + updated_guessed_word[i+1:]
...
while guesses != 0 and guessed_word != hidden_word:
